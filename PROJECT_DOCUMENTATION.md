...
My project meets all function and non functional requirements I had written

2. Analyse the performance of your program against the key use-cases you identified.
The performance is fine but can sometimes lag when encountering an enemy

3. Assess the quality of your code in terms of readability, structure, and maintainability.
The readability and structure is really good using classess and comments to see what that part of the code does.

4. Explain the improvements that should be made in the next stage of development.
Maybe adding special attack for the player.

---
## Evaluation of System
1. \
    a. **Explain how you could improve your system in future updates.** \
    The system could be enhanced by adding special class abilities, adding more weapons and equipments, a shop and a better GUI. All this would significantly improve the users interaction with the game. The enhancements would increase the strategy in the game as well as playability.

    b. **Analyse the impact these updates could have on the user experience.**\
    The proposed combat enhancements would create more tactical variety, appealing to players who enjoy strategic decision-making. Special abilities and status effects would add layers of complexity while systems like the shop would allow purchase of items that may be hard to get from exploring rooms.

    Expanded dungeon generation would significantly improve exploration. Procedural biomes and interactive environments would make each playthrough feel unique, while puzzle rooms and shops would break up combat monotony. This variety would maintain player engagement across multiple runs.


2.  **Evaluate the system in terms of how well it meets the requirements and specifications.**\
    The system fully delivers core functional requirements including dungeon navigation, turn-based combat, character progression, and inventory management. The class system provides distinct playstyles while the boss battles demonstrate effective implementation of special abilities.

    Visually, the game meets non-functional requirements with clear feedback and responsive controls. The minimap effectively communicates spatial relationships while color-coding (red for enemies, yellow for keys) creates intuitive understanding. Performance remains stable during testing.

    Some minor gaps exist in accessibility options and persistence. While not critical, features like key rebinding or save slots would improve quality-of-life. The help menu explains mechanics but an interactive tutorial could better onboard new players.

    Overall, the implementation successfully fulfills the specified requirements while maintaining extensibility. The modular architecture allows for straightforward addition of the identified improvements without major restructuring.

3. **Evaluate your processes in terms of project management.**\
    The project demonstrates strong adherence to software engineering principles through its organized structure. Clear separation between game systems (combat.py, dungeon.py, etc.) enabled parallel development and simplified debugging. Version control practices are evident in the coordinated file organization.

    Documentation through docstrings and consistent naming conventions supports maintainability. However, more formal design documents could have further streamlined collaboration, especially for larger teams. The lack of unit testing represents a missed opportunity for quality assurance.

    The object-oriented approach proved effective for this scope. Encapsulation prevented side effects while inheritance reduced code duplication. Little errors from GUI and game classes exists but doesn't significantly impact current functionality.

4. **Gather feedback from at least two peers on meeting of functional and non-functional specifications.**\
    Stephan:
    The combat system is great however the UI could be more visually engaging. Character progression through equipment and buffs works well. Expanded inventory sorting options could be improve. 

    Barry:
    There could be more improvement on the combat system as the attack button doesn't work but can use [a] instead so the combat ystem still works. Could improve on the User inteface as it is a bit bland.

5. **Justify your use of OOP class features**\
    The inheritance hierarchy between Enemy and Boss demonstrates effective use of polymorphism. Shared attributes like health and attack patterns are defined in the parent class while special abilities are cleanly extended in the subclass. This allows the combat system to interact with all enemies through a consistent interface.

    Encapsulation protects game state integrity. The Player class manages health, inventory and position through controlled methods rather than direct variable access. This prevents invalid states like negative health while allowing internal calculations (e.g. total_attack()) to remain hidden.

    The single responsibility principle shines in classes like Room and Dungeon. Room handles local state (items, enemies) while Dungeon manages floor generation and navigation. This separation allowed independent development of dungeon features without impacting room behaviors.

    These OOP implementations directly support maintainability and extensibility. New enemy types can be added without modifying combat logic, and inventory expansions won't affect character stats. The architecture proves particularly valuable for iterative development of this genre.